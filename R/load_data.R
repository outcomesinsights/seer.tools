...
#' s <- setup_incidence()
#' seer <- load_files("LYMYLEUK", s$datafiles)
#' aml <-
#'     setDT(seer) %>%
#'     setkey(., siterwho) %>%
#'     .[J(35021)] %>%
...
#' }
#' @export
load_files <- function(fname = ".", vec_of_files) {
    x <-
        lapply(
            vec_of_files[grepl(fname, vec_of_files, ignore.case = TRUE)],
            function(x) {
                readr::read_fwf(x, input_specs, progress = FALSE)
            }
            )
...
setup_incidence <- function(inc_dir = "./data/raw/"){
    ...
    details <- list(coltype, input_specs, datafiles)
    return(details)
}
